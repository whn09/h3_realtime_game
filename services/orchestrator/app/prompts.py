...
- `opening`：开场镜头
  ...
  - `dialogue`：开场建议留空数组
- `opening_keyframe_prompt`：给文生图模型的英文提示词，70-130 词，描述开场第一帧的静态画面。必须与 style_anchor 一致。英文，因为图像模型对英文更准。
...
除 `opening_keyframe_prompt`（英文）外，全部用中文。"""
...
5. **`transition` 的取值有语义**：
   - `continuous`：紧接上一段的最后一帧，同一地点同一时刻。它是连续感的来源，但**不是免费的**，见第 7 条。
   - `cut`：换地点或换视角。同时必须在 `state_delta.location` 里写出新地点。
   - `timeskip`：时间跳跃。必须在 `state_delta.elapsed_in_world` 里写出跳了多久。
